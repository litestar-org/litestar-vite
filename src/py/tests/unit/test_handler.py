...
async def test_spa_handler_dev_mode_proxy(spa_config_dev: ViteConfig, mocker: "MockerFixture") -> None:
    ...
    fetch_mock = Mock(return_value="<html><head></head><body>Dev Server HTML</body></html>")
    with patch.object(AppHandler, "_fetch_vite_url_sync", fetch_mock):
        ...
        fetch_mock.assert_called_once_with(f"{expected_url}/")
...
async def test_spa_handler_dev_mode_proxy_error(spa_config_dev: ViteConfig) -> None:
    ...
    with patch.object(AppHandler, "_fetch_vite_url_sync", Mock(side_effect=OSError("Connection refused"))):
        await handler.initialize_async(vite_url="http://127.0.0.1:5173")
...
async def test_spa_handler_shutdown(spa_config_dev: ViteConfig) -> None:
    """Test SPA handler shutdown completes cleanly."""
    handler = AppHandler(spa_config_dev)
...
async def test_spa_handler_get_html_sync_works_in_dev_mode(spa_config_dev: ViteConfig) -> None:
    """Test that get_html_sync works in dev mode."""
    handler = AppHandler(spa_config_dev)

    fetch_mock = Mock(return_value="<html><head></head><body>Dev Mode</body></html>")
    with patch.object(AppHandler, "_fetch_vite_url_sync", fetch_mock):
        await handler.initialize_async(vite_url="http://127.0.0.1:5173")
...
async def test_proxy_to_dev_server_returns_starting_html_on_error(spa_config_dev: ViteConfig) -> None:
    ...
    with patch.object(AppHandler, "_fetch_vite_url_sync", Mock(side_effect=OSError("boom"))):
        await handler.initialize_async(vite_url="http://127.0.0.1:5173")
...
async def test_spa_handler_route_exclusion_dev_mode(spa_config_dev: ViteConfig) -> None:
    ...
    with patch.object(
        AppHandler, "_fetch_vite_url_sync", Mock(return_value="<html><head></head><body>Dev Server HTML</body></html>")
    ):
        await handler.initialize_async(vite_url="http://127.0.0.1:5173")
