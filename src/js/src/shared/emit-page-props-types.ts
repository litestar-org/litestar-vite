...
`
  ...
  // Inertia carries flash on the page object (`page.flash`), typed by its own
  // `flashDataType` config slot which defaults to `{ [key: string]: unknown }`. Declaring
  // FlashMessages there is what makes `page.flash.success` a `string[]` rather than
  // `unknown`. Only emitted alongside the FlashMessages interface it references.
...
`
    ...
    // Stale declarations stay ambient and keep applying, so remove rather than leave behind.
