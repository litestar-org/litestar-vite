"""Svelte 5 + Inertia.js example with a Jinja2 page shell.

Mirrors ``examples/react-inertia-jinja`` and ``examples/vue-inertia-jinja``
for Svelte 5 + ``@inertiajs/svelte@3``. The Jinja2 ``templates/index.html``
shell renders the page payload through the stable script-element bootstrap
(``<script id="app_page" data-page="app">``).
"""
...
class LibraryController(Controller):
    ...
    async def index(self) -> Message:
        ...
        Returns:
            The result.
        """
        return Message(message="Welcome to Svelte Inertia (Jinja shell)!")
...
vite = VitePlugin(
    config=ViteConfig(
        ...
        runtime=RuntimeConfig(port=5024),
    )
)
