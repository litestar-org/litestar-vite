"""SPA React + React Router example — single-port demonstration.
...
This example differs from ``examples/react`` by using the React Router 7
data router (``createBrowserRouter``) instead of declarative ``<Routes>``.
``mode="spa"`` plus ``litestar-vite`` means the browser only ever sees the
Litestar port — Vite's HMR and chunks are proxied through ``/static/...``.
"""
...
vite = VitePlugin(
    config=ViteConfig(
        ...
        types=TypeGenConfig(),
        runtime=RuntimeConfig(port=5004),
    )
)
