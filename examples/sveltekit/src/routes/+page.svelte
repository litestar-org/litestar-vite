...
<main class="mx-auto max-w-5xl space-y-6 px-4 py-10">
  <header class="space-y-2">
    ...
    <h1 class="text-3xl font-semibold text-[#202235]">Library (SvelteKit)</h1>
    ...
    <nav class="inline-flex gap-2 rounded-full bg-slate-100 p-1 shadow-sm" aria-label="Views">
      <button
        class="rounded-full px-4 py-2 text-sm font-semibold transition {view === 'overview' ? 'bg-white text-[#202235] shadow' : 'text-slate-600'}"
        onclick={() => (view = "overview")}
      >
        Overview
      </button>
      <button
        class="rounded-full px-4 py-2 text-sm font-semibold transition {view === 'books' ? 'bg-white text-[#202235] shadow' : 'text-slate-600'}"
        onclick={() => (view = "books")}
      >
  ...
  {#if view === "overview"}
    <section class="space-y-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200/40">
      <h2 class="text-xl font-semibold text-[#202235]">{data.summary.headline}</h2>
